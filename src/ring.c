...
void ring_destroy(ring_t *r) {
    // Detach state under lock, then free outside lock (avoids free() under spinlock)
    uint32_t save = spin_lock_blocking(r->core.spin_lock);
    uint8_t *data = r->data;
    r->data = NULL;
    r->size = r->used = r->wptr = r->rptr = 0;
...
ring_write_internal(ring_t *r, const void *ptr, uint16_t len, bool block) {
    ...
    do {
        uint32_t save = spin_lock_blocking(r->core.spin_lock);
        assert(r->used <= r->size && r->wptr < r->size && r->rptr < r->size);
